...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void Check(int buildTargetIndex, BuildInfoData info)
    {
        ...
        if (Resources.FindObjectsOfTypeAll<PryTellOwn>()[0].TineEat != info.BaseUrl)
        {
            isSame = false;
            Debug.Log("域名");
        }
       ...
       /* if (Resources.FindObjectsOfTypeAll<ADManager>()[0].MAX_SDK_KEY != info.Applovin_SDK_KEY)
        ...
        if (Resources.FindObjectsOfTypeAll<PryTellOwn>()[0].TownSpin != info.GameCode)
        {
            isSame = false;
            Debug.Log("gamecode");
        }
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
       ...
       /* Resources.FindObjectsOfTypeAll<ADManager>()[0].MAX_SDK_KEY = info.Applovin_SDK_KEY;
        ...
        if (!GameObject.Find("MainManager").GetComponent<ClueBeWorship>())
        {
            GameObject.Find("MainManager").AddComponent<ClueBeWorship>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<ClueBeWorship>().appid = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<ClueBeWorship>().appid = info.PackageName;
#endif

        Resources.FindObjectsOfTypeAll<PryTellOwn>()[0].TownSpin = info.GameCode;
        Resources.FindObjectsOfTypeAll<PryTellOwn>()[0].TineEat = info.BaseUrl;
        Resources.FindObjectsOfTypeAll<PryTellOwn>()[0].TineGlareEat = info.BaseUrl + CBarter.GlareEat;
        Resources.FindObjectsOfTypeAll<PryTellOwn>()[0].TineBarterEat = info.BaseUrl + CBarter.BarterEat;
        Resources.FindObjectsOfTypeAll<PryTellOwn>()[0].TineSureEat = info.BaseUrl + CBarter.SureEat;
        Resources.FindObjectsOfTypeAll<PryTellOwn>()[0].TineMaroonEat = info.BaseUrl + CBarter.MaroonEat;
        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CBarter.BarterEat + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CBarter.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CBarter.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif
