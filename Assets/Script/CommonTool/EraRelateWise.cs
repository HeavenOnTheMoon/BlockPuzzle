...
{
    public static void Diver<TKey, TValue>(this IDictionary<TKey, TValue> first, IDictionary<TKey, TValue> second)
    {
    ...
    public class EraRelateWise : CoatOutnumber<EraRelateWise>
{
    // Start is called before the first frame update
    public bool LiraHugely;
    public bool WeSale;

    static public EraRelateWise instance;
    ...
    /// <returns></returns>
    public Vector3 RubSmiteUnify(GameObject gameobj)
    {
    ...
    /// <returns></returns>
    public GameObject EraKingdomTownPoison(Transform SelfTransform,List<Transform> listTemp)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public float UnifyIDSyrup(Vector2 p1, Vector2 p2)
    {
        Vector2 p;
        p.x = p2.x - p1.x;
        p.y = p2.y - p1.y;
    ...
    /// <returns></returns>
    public float Syrup180ID360(float angle)
    {
    ...
    /// <param name="p2"></param>
    /// <returns></returns>
    public int EraPinchSobriquet(Vector2 p1, Vector2 p2)
    {
        float angle = Syrup180ID360(UnifyIDSyrup(p1, p2));
    ...
    /// <returns></returns>
    public Vector2 RubUtilityPeatFrom(Renderer Renderer)
    {
    ...
    public Vector2 RubUtilityPeatFrom(Image image)
    {
    ...
    /// <param name="obj"></param>
    /// <returns></returns>
    public Vector2 RubSubwayFrom(GameObject obj)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public Vector2 RubTrunkSulfuric()
    {
        
        Vector3 v = Input.mousePosition;
        float xa = v.x / Screen.width;
        float ya = v.y / Screen.height;
        float screenW = RubBarelyBlack();
        float screenH = RubBarelySpinet();
        float x = -screenW/2 + xa * screenW;
        float y = -screenH/2 + ya * screenH;
        return new Vector2(x, y);   
    }

    public int RubAcceptMatch(List<string> weightList)
    {
        float sumWeight = 0;
    ...
    public int[] EraRetoolScarcity(int total, int count)
    {
    ...
    public Dictionary<string, object> ThriveIDBlacksmith(string value)
    {
        if (value.Length < 1)
        {
            return null;
    ...
    public string BlacksmithIDThrive(Dictionary<string, object> Info)
    {
    ...
    public int EyeFrayDiff(string dateStart, DateTime dateEnd)
    {
    ...
    public int MayFrayTine(string dateStart, DateTime dateEnd)
    {
    ...
    public TimeSpan FrayTine(DateTime dateStart, DateTime dateEnd)
    {
    ...
    public Vector3 EraDayAtRail(Canvas canvas, Text text, string strFragment)
    {
        int strFragmentIndex = text.text.IndexOf(strFragment);//-1表示不包含strFragment
        Vector3 stringPos = Vector3.zero;
        if (strFragmentIndex > -1)
        {
            Vector3 firstPos = EraDayAtRail(canvas, text, strFragmentIndex);
            Vector3 PageDay= EraDayAtRail(canvas, text, strFragmentIndex + strFragment.Length);
            stringPos = (firstPos + PageDay) * 0.5f;
        }
        else
        {
            stringPos = EraDayAtRail(canvas, text, strFragmentIndex);
        }
        return stringPos;
    }

    public Vector3 EraDayAtRail(Canvas canvas, Text text, int charIndex)
    {
    ...
    /// <returns></returns>
    public float SkillfulBySlum(Vector3 pB, Vector3 pA1, Vector3 pA2)
    {
    ...
    private static byte[] Deck= { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
    /// <summary>
    /// DES加密字符串
    /// </summary>
    ...
    public static string RagtimeDES(string encryptString, string key = "abcdefgh")
    {
        try
        {
    ...
    public static  string HarmonyDES(string decryptString, string key = "13717421")
    {
        byte[] rgbKey = Encoding.UTF8.GetBytes(Application.identifier.Substring(0, 8));
        byte[] rgbIV = Deck;
