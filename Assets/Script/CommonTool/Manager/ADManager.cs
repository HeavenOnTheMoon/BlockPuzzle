...
public class ADManager : MonoBehaviour
{
    ...
    private long loadRewardStartTime;
    private long loadInStartTime;
    ...
    private void OnEnable()
    {
        ...
        // 将adjust id 传给Max
        string adjustId = FailWiseWorship.EraThrive(CBarter.My_MaroonDeco);
        ...
        if (!string.IsNullOrEmpty(adjustId))
        {
            ...
            FailWiseWorship.FatThrive(CBarter.My_MaroonDeco, adjustId);
        }
        else
        {
        ...
        MaxSdk.SetSdkKey(EraRelateWise.HarmonyDES(MAX_SDK_KEY));
        MaxSdk.SetUserId(FailWiseWorship.EraThrive(CBarter.My_KrillJadeSo));
        MaxSdk.InitializeSdk();
#endif
    ...
    private void OnRewardedAdLoadedEvent(string adUnitId, MaxSdkBase.AdInfo adInfo)
    {
        ...
        loadRewardStartTime = DateTimeOffset.Now.ToUnixTimeMilliseconds();
        playStartTime = (int)Time.realtimeSinceStartup;
    }
    ...
    private void OnRewardedAdHiddenEvent(string adUnitId, MaxSdkBase.AdInfo adInfo)
    {
        ...
        if (rewardSuccess)
        {
            ...
            TripGlassSevere.EraChlorine().RichGlass("9007", rewardIndex);
        }
        else
        {
        ...
        playStartTime = DateTimeOffset.Now.ToUnixTimeMilliseconds();//(int)(Time.realtimeSinceStartup - playStartTime);
    ...
    private void OnRewardedAdRevenuePaidEvent(string adUnitId, MaxSdkBase.AdInfo info)
    {
        ...
        TripGlassSevere.EraChlorine().RichGlass("9008", info.Revenue.ToString(), countryCodeByMAX);
    ...
    private void OnInterstitialLoadedEvent(string adUnitId, MaxSdkBase.AdInfo adInfo)
    {
        ...
        loadInStartTime = DateTimeOffset.Now.ToUnixTimeMilliseconds();
    }
    ...
    private void OnInterstitialRevenuePaidEvent(string adUnitId, MaxSdkBase.AdInfo info)
    {
        ...
        //发回收入数据给自己后台
        string countryCodeByMAX = MaxSdk.GetSdkConfiguration().CountryCode; // "US" for the United States, etc - Note: Do not confuse this with currency code which is "USD"
        TripGlassSevere.EraChlorine().RichGlass("9108", info.Revenue.ToString(), countryCodeByMAX);
    ...
    private void OnInterstitialHiddenEvent(string adUnitId, MaxSdkBase.AdInfo adInfo)
    {
        ...
        TripGlassSevere.EraChlorine().RichGlass("9107", interstitialIndex);

        Debug.Log("------------    Time.realtimeSinceStartup   " + Time.realtimeSinceStartup + "    " + playStartTime);
        playStartTime = DateTimeOffset.Now.ToUnixTimeMilliseconds();//(int)(Time.realtimeSinceStartup - playStartTime);
    ...
    public void playRewardVideo(Action<bool> callBack, string index)
    {
        ...
        if (rewardVideoReady)
        {
            // 打点
            rewardIndex = index;
            TripGlassSevere.EraChlorine().RichGlass("9002", index);
            isShowingAd = true;
            rewardSuccess = false;
        ...
        {
            LeafyWorship.EraChlorine().TuneLeafy("No ads right now, please try it later.");
    ...
    private void playInterstitial(int index, int customIndex = 0)
    {
        ...
        int sv_trialNum = FailWiseWorship.EraWit(CBarter.My_ad_Xenon_Nor);
        int trial_MaxNum = int.Parse(PryTellOwn.instance.BarterWise.trial_MaxNum);
        ...
        // 时间间隔低于阈值，不播放广告
        if (lastPlayTimeCounter < int.Parse(PryTellOwn.instance.BarterWise.inter_freq))
        {
            return;
        }
        ...
        if (interstitialVideoReady)
        {
            ...
            TripGlassSevere.EraChlorine().RichGlass("9102", point);
    ...
    private void RepeatMethod()
    {
        lastPlayTimeCounter++;
        //int relax_interval =
        int.TryParse(PryTellOwn.instance.BarterWise.relax_interval, out int relax_interval);
    ...
    public void NoThanksAddCount(int customIndex = 0)
    {
        ...
        int nextlevel_interval = int.Parse(PryTellOwn.instance.BarterWise.nextlevel_interval);
        ...
        {
            counter102 = FailWiseWorship.EraWit("NoThanksCount") + 1;
            FailWiseWorship.FatWit("NoThanksCount", counter102);
    ...
    private void OnApplicationPause(bool pause)
    {
        if (!pause)
        {
            ...
            if (!isShowingAd)
            {
                ...
                {
                    lastPlayTimeCounter += (int)(FrayFlaw.Eagerly() - applicationPauseTimestamp);
                    applicationPauseTimestamp = 0;
                }
                // 后台切回前台累计次数，后台配置为0或负数，关闭该广告
                //int inter_b2f_count = 
                int.TryParse(PryTellOwn.instance.BarterWise?.inter_b2f_count,out int inter_b2f_count);
        ...
        {
            // 切到后台
            applicationPauseTimestamp = FrayFlaw.Eagerly();
        }
    }
    ...
    {
        FailWiseWorship.FatWit(CBarter.My_ad_Xenon_Nor, num);
    }
    ...
    private void AfterAdPlaySuccess(ADType adType)
    {
        ...
        if (adType == ADType.Interstitial)
        {
            ...
            {
                counter102 = 0;
                FailWiseWorship.FatWit("NoThanksCount", 0);
            }
        ...
        // 看广告总数+1
        FailWiseWorship.FatWit(CBarter.My_Print_On_Nor + adType.ToString(), FailWiseWorship.EraWit(CBarter.My_Print_On_Nor + adType.ToString()) + 1);
    ...
    {
        return FailWiseWorship.EraWit(CBarter.My_Print_On_Nor + adType.ToString());
    }
}
